...
import { INIT_OPTI_TIME_NUMBER_AVG_SERIE_FUNCTION } from '../../ts/chart/init-opti-time-number-avg-serie-function.util.js'
import { mockDateTime } from '../mocks/luxon-mock.js'

describe(
  'INIT_OPTI_TIME_NUMBER_AVG_SERIE_FUNCTION',
  () => {
    it(
      'should create an array of [timestamp, null] pairs for a single step',
      async () => {
        const mFrom = mockDateTime(1000)
        const mTo = mockDateTime(1000)
        const step = { milliseconds: 1000 }

        const result = await INIT_OPTI_TIME_NUMBER_AVG_SERIE_FUNCTION({
          mFrom,
          mTo,
          step,
        })

        expect(result).toEqual([[1000, null]])
      }
    )
    ...
    it(
      ...
      async () => {
        ...
        expect(result).toEqual([[100000, null]])
      }
    )
    ...
    it(
      ...
      async () => {
        ...
        expect(result).toEqual([[2000, null]])
      }
    )
